...
const FlashcardList = ({
  ...
  const newFlashcards = flashcards.filter(
    (f) => getClassForFlashcard(f) === ""
  );
  const importantFlashcards = flashcards.filter(
    (f) => getClassForFlashcard(f) === "Important"
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        <h2
          className={`text-lg font-medium text-[#071434] ${
            newFlashcards.length > 0 ? "mt-[100px]" : "mt-[20px]"
          }`}
        >
          Important Cards
        </h2>
      ...
      <div>
        <div className="bg-white shadow-sm border border-gray-200 rounded-xl mt-20 pb-3">
          <h2 className="text-lg font-medium text-[#071434] my-4 ml-4">
            Reviewed
          </h2>
          <table
          ...
          >
            <thead>
              <tr className="bg-[#FCFCFC] border-b border-gray-200">
                <th className="px-4 py-2 text-left text-[13px] text-[#4B5675] font-normal">
                  Heading
                </th>
                <th className="px-4 py-2 text-left text-[13px] font-normal text-[#4B5675]">
                  Description
                </th>
                <th className="px-4 py-2 text-left text-[13px] font-normal text-[#4B5675]">
                  Classification
            ...
            <tbody>
              {reviewedFlashcards.map((flashcard, index) => (
                ...
                >
                  <td className="border-t px-4 py-2 text-sm text-normal text-left">
                    {flashcard.heading}
                  </td>
                  <td className="border-t px-4 py-2 text-[12px] text-left">
                    {flashcard.description}
                  </td>
